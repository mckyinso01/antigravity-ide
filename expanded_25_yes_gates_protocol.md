...
```mermaid
graph TD
    A[Task Execution Started] --> B[State 1: Builder Composition]
    B --> C[State 2: Adversarial Critic Self-Audit]
    C --> D{Check All 25 YES-Gates}
    D -- Any NO Answer --> E[Iterate, Refactor & Polish]
    E --> B
    D -- 25 / 25 YES --> F[Submit to Council of Elders]
    F --> G{Unanimous 6-Elder Approval?}
    G -- Rejection --> E
    G -- Approved --> H[Master Orchestrator Final Attestation]
```
