...
5. **Mandatory 120+ Exhaustive Scenarios Rule**: Every project MUST formulate at least 120 Exhaustive User Journey Scenarios (30 per component across 4 core components) covering Reporting, Analytics, Automations, Views, Modals, and Drawers before code drafting.
6. **Mandatory 18-Council & MCP Invocation Protocol**: Across every stage, the Orchestrator MUST formally invoke and cross-examine with the 18 Domain Council Subagents (`SEC-01`, `FE-01`, `QA-01`, `ARCH-01`, `SRE-01`) and all registered MCP servers (`StitchMCP`, `chrome-devtools-mcp`).
...
## Mandatory Dynamic GitHub Models Search & 4-Brain Verification Protocol for Novel Tasks

Whenever a novel, non-standard, or specialized task arrives that does not match predefined templates:
1. **GitHub Catalog Search Mandate**: The Orchestrator MUST dynamically query GitHub Models (`https://models.github.ai/catalog/models`) to discover the top 4 AI models specialized for that exact task domain.
2. **4-Brain Alignment Verification**: The Orchestrator MUST verify 100% domain match across all 4 selected AI Brains before proceeding.
3. **Execution Clearance Gate**: The 4-Brain alignment receipt MUST be confirmed and presented to the user BEFORE starting any code drafting or task execution.
