...
32. **Autonomous Sentinel Verification Council Directive (`AUTONOMOUS-SENTINEL-VERIFICATION-COUNCIL`)**: The Orchestrator MUST execute the Autonomous Sentinel Verification Council script (`python .agents/scripts/sentinel_checklist.py`) on EVERY turn before finalizing output to verify Council Swarm invocation, Master Ledger synchronization, 20-issue milestone threshold check, 100% CLI audit pass, and Git workspace cleanliness.
...
36. **Verified Client Discovery & Live SMTP Outreach Protocol (`VERIFIED-CLIENT-OUTREACH-STANDARD`)**: Every product release MUST undergo Stage 12 Post-Production Client Outreach: (a) Scrape/discover 100 real business client inboxes, (b) Dual-verify email format and domain MX records, (c) Dispatch 11-feature formal English executive proposal pitches via Gmail SMTP TLS (`send_authenticated_gmail.py`), and (d) Execute IMAP SSL bounce audits (`check_gmail_bounces.py`) to verify >90% inbox landing rate.
