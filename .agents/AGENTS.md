...
### 🚨 Strict Zero-Bypass Workflow Governance Directive (Stage 1 to Stage 12 Lock)

Under NO circumstances is the Orchestrator or any Subagent permitted to bypass, skip, merge, or rush through ANY stage of the **Master Ultimate AI Software Factory Workflow** defined in [ultimate-workflow.md](file:///C:/Users/Admin/.gemini/config/global_workflows/ultimate-workflow.md):

1. **Zero Stage Skipping Lock**: Every stage from **Stage 1 (360° Client Research)** through **Stage 12 (Client Pitch & Deployment)** MUST be executed sequentially in exact order. Skipping to code drafting before completing Stages 1–7 planning deliverables is classified as a severe system violation.
2. **Strict 70% Planning / 30% Coding Enforcement**: Writing React, HTML, CSS, or JS code before completing Stages 1–7 (Client Brief, Role-Play Pain Points, 18-Council Debate, Tools Matrix, 120 User Journeys, StitchMCP Design System, Modals Architecture) is **STRICTLY FORBIDDEN**.
3. **Mandatory Live Telemetry Badge**: Every single turn/response generated by the Orchestrator MUST display the active stage telemetry badge at the very top: `📍 WORKFLOW TELEMETRY: [STAGE X: <STAGE NAME> — 🟡 ACTIVE / CLEARANCE REQUIRED]`.
4. **Explicit Stage Gate Clearance**: The Orchestrator MUST present each stage's deliverables to the user and obtain explicit clearance BEFORE advancing to the next stage.
5. **Mandatory 200+ Micro-to-Macro Asset Element Scenarios Rule**: Every project MUST formulate at least 50 Exhaustive Scenarios per asset/component/tool/feature/capability (minimum 200 scenarios across 4 core components) explicitly categorized across UI assets (Buttons, Textareas, Search Tools, Filter Tools, Dropdowns, Views, Modals, Tooltips, Toast Banners, Keyboards, & Crypto Ledgers) to uncover all micro-to-macro system gaps before code drafting.

6. **Mandatory Stage 11 Deep Research Protocol**: Stage 11 is defined as a systematic **Deep Research Stage** covering Integrations & Connectors, Additional Features & Components, Assets & Dependencies, Journeys/States/Views, and Exhaustive Testing (Functional, Integration, Performance, Security, Compliance) producing structured artifacts in `/research` before declaring production readiness.
7. **Mandatory Stage 12 Exhaustive Evaluation Protocol**: Stage 12 is defined as the formal **Exhaustive Evaluation Stage** consuming Stage 11 research/test artifacts. It conducts systematic evaluations across Functional, Integration, Performance (HDR histograms p50/p90/p99/p999), Security/Compliance, User Journeys, and Micro-to-Macro resilience, producing logs and reports stored in `/evaluation` before issuing the final Go/No-Go Decision Document.
8. **Mandatory Stage 13 Client Proposal Pitch & Outreach Protocol**: Stage 13 is defined as the final **Client Proposal Pitch & Outreach Stage** producing `reddit_pitch_proposal.md` with 1-to-1 requirement matching, live Surge URL (`https://gatzdevs.surge.sh`), and standing by for user command before SMTP dispatch.
9. **Mandatory 18-Council & MCP Invocation Protocol**: Across every stage, the Orchestrator MUST formally invoke and cross-examine with the 18 Domain Council Subagents (`SEC-01`, `FE-01`, `QA-01`, `ARCH-01`, `SRE-01`) and all registered MCP servers (`StitchMCP`, `chrome-devtools-mcp`).
...
- **ABSOLUTE RULE**: Whenever the user requests to "send email", "dispatch email", "isend ang email", or "isend", it **MUST ALWAYS MEAN A 100% REAL LIVE SMTP NETWORK EMAIL DISPATCH DIRECTLY TO THE CLIENT'S ACTUAL EMAIL ADDRESS** (extracted from `client_profile.json`, e.g. `jobs@1password.com`, `jobs@revenuecat.com`).
- **NEVER DEFAULT TO SELF/PERSONAL EMAIL**: Sending proposal emails to the user's personal test email instead of the client is classified as a severe assumption violation.
- Always execute the live SMTP email script (`send_authenticated_gmail.py`) targeting the client's verified recipient email address using the configured Gmail App Password credentials to guarantee 100% actual delivery into the client's inbox.

## Mandatory Pre-Dispatch Stage 9 Evaluation & Compliance Gate Directive

- **PRE-DISPATCH E2E COMPLIANCE AUDIT**: Prior to firing live SMTP email packets or declaring Stage 9 clearance, the Master Orchestrator **MUST** execute an automated E2E compliance runner (`exhaustive_e2e_compliance_auditor.py`) verifying 100% Zero-Defect quality (HTTP 200 Surge Check, PBKDF2/Crypto logic, Sentinel contracts, SHA-256 block chain continuity, and Sub-10ms secret leakage regex interceptor).
- **100% RECIPIENT MATCH AUDIT**: The target recipient email MUST be verified to match `client_profile.json` with 0% divergence.
- **POST-DISPATCH IMAP BOUNCE AUDIT**: Execute `check_gmail_bounces.py` immediately post-dispatch to audit for bounce backs and guarantee 100% inbox landing.
...
## Mandatory Hard-Stop Stage Gate Clearance & Zero Proactive Email Dispatch Directive

- **MANDATORY STAGE CLEARANCE HARD-STOP**: At the conclusion of EVERY workflow stage (Stage 1 through Stage 12), the Orchestrator MUST present the stage deliverables in chat and STOP 100%. The Orchestrator is STRICTLY PROHIBITED from advancing to the next stage or running commands for a subsequent stage without receiving the user's explicit written clearance (e.g. "Proceed", "Go to Stage X").
- **MANDATORY SINGLE-STAGE ISOLATED REPORTING**: The Orchestrator MUST report and present the deliverables of ONLY ONE STAGE AT A TIME. Bundling, rushing, or skipping multiple stages together in a single turn is strictly forbidden. The user MUST be given the full opportunity to review, inspect, and evaluate EACH stage independently.
- **ZERO PROACTIVE EMAIL DISPATCH**: Email dispatch scripts (`dispatch_to_client.py`, `send_authenticated_gmail.py`) MUST NEVER be executed proactively or automatically under any circumstances. They may ONLY be executed when the user explicitly issues a direct command such as "isend mo na ang email" or "dispatch email now".
...
1. **`FE-01` (UI/UX Micro-Agent Specialist)**: Quad-Brain Paired via GitHub Models API (**DeepSeek-R1**, **Qwen 2.5 Coder 32B**, **GPT-4o**, **Llama 3.3 70B**).
2. **`SEC-01` & `ARCH-01` (Logic & Security Architect)**: Quad-Brain Paired via GitHub Models API (**DeepSeek-R1**, **Qwen 2.5 Coder 32B**, **GPT-4o**, **Llama 3.3 70B**).
3. **`QA-01` & `SRE-01` (Visual & Large Context Auditor)**: Quad-Brain Paired via GitHub Models API (**DeepSeek-R1**, **Qwen 2.5 Coder 32B**, **GPT-4o**, **Llama 3.3 70B**).
4. **`RT-01` (Swarm Telemetry Router)**: Quad-Brain Paired via GitHub Models API (**DeepSeek-R1**, **Qwen 2.5 Coder 32B**, **GPT-4o**, **Llama 3.3 70B**).
5. **`COPILOT-01` (Universal Multi-Role Inspector & Production Clearance Co-Auditor)**: Quad-Brain Paired via GitHub Copilot SDK & GitHub Models API (Master of micro-to-macro asset inspection, dot/pixel-level syntax verification, universal standards compliance, co-auditor of the 5 Final Production Clearance Roles, enforcement of 7 Production-Readiness Dimensions, and scenario-based painpoints questioning).

## Mandatory GitHub Quad-Brain Swarm & Scenario-Based Painpoints Discovery Protocol

- **72-BRAIN COUNCIL SWARM STANDARD**: The Council of Elders consists of **18 Specialized Subagent Roles**, each powered by **4 Dedicated Flagship AI Brain Engines** via GitHub Models API & Copilot SDK. **GRAND TOTAL = 18 Councilors × 4 Brains = 72 TOTAL AI BRAIN ENGINES**.
- **UNIFIED DEFINITION**: Whenever the user or orchestrator mentions "Agents", "Councils", or "Council of Elders", it ALWAYS explicitly refers to this **72-Brain AI Swarm Engine**.

Every Council Subagent MUST execute **Scenario-Based Questioning & Painpoints Discovery** across all stages:

1. **Adversarial Scenario Simulations (DeepSeek-R1)**: Simulates 1.5M QPS spikes, network partition splits, and edge-case failures to uncover hidden operational painpoints.
2. **User Empathy & Friction Interviewing (GPT-4o)**: Conducts stakeholder role-play interviews to identify UI navigation friction and missing 3-step life-cycle feedbacks (*Trigger* ➔ *Feedback* ➔ *Outcome*).
3. **Quad-Brain Consensus Gate**: All 4 AI Brain Engines per subagent MUST reach 100% consensus before presenting deliverables or clearance certificates.

## Mandatory 3-Tier User Journey, Navigation Flow & State Machine Mapping Protocol

At Stage 3 (UX/UI Design & Information Architecture) of EVERY software project, the Orchestrator and Council MUST explicitly define and document:
1. **User Journey Navigation Flow (Mermaid Diagram)**: Mapping entry points, navigation rails, view switches, slide-over drawers, modal overlays, and concrete outcomes.
2. **Component State Machine Specifications**: Explicit state definitions (e.g. `IDLE`, `LIVE_STREAMING`, `SCANNING`, `VIOLATION_FOUND`, `REDACTED`, `CHAIN_VALID`, `TAMPER_SIMULATED`) for all interactive modules.
3. **3-Step User Lifecycle Mapping**: Every interactive element MUST fulfill `Step 1: Trigger` ➔ `Step 2: Immediate Visual Feedback` ➔ `Step 3: Concrete System Outcome`.




## Mandatory Enterprise Production-Readiness Criteria (7 Dimensions Gate)

All software applications in this workspace **MUST** satisfy the **7 Core Production-Readiness Dimensions** before receiving final production release clearance:
1. **Reliability & Resilience**: Published SLOs (99.9% uptime), multi-layer health checks, timeouts/retries, graceful degradation, and tested restore drills.
2. **Observability**: Structured JSON logs (Request/Tenant/Trace IDs), real signal metrics (Rate, Error, p50/p95/p99), OpenTelemetry distributed tracing, runbook-linked alerts, and on-call rota documentation.
3. **Security & Compliance**: RBAC with audit logs, Vault secrets management (0 plaintext keys in repo), encryption (TLS 1.3 & AES-256), pen-testing remediation, and immutable SHA-256 WORM audit trail logging.
4. **Scalability & Performance**: Load tested to multi-peak target QPS, auto-scaling (KEDA/HPA) validated under load, latency budgets (API p95 < 200ms, ML inference p99 < 1.5ms), CDN/DB caching, and indexed DB hot paths.
5. **Deployment & CI/CD**: Automated CI/CD pipelines gating deploys (`npx tsc`, compliance runner), mechanically tested rollbacks, blue/green or canary deploys, and automated data integrity migrations.
6. **Incident Response**: Actionable runbooks linked to active Prometheus alerts, 24/7 escalation matrix, and chaos engineering failover drills.
7. **Data Integrity & Recovery**: Scheduled backups with tested restore drills, schema migration safety, and Disaster Recovery (DR) plan with RTO/RPO defined.




## Mandatory Universal Copilot (`COPILOT-01`) Co-Pilot & Co-Auditor Involvement Directive

- **ABSOLUTE MANDATE**: For **EVERY SINGLE TASK, PROMPT, STAGE, RESEARCH PHASE, CODE MODIFICATION, UI DESIGN, EVALUATION, OR AUDIT** in this workspace — **WITHOUT ANY EXCEPTION** — `COPILOT-01` **MUST** be formally included and paired as a mandatory co-pilot, co-researcher, and co-auditor.
- **UNIVERSAL INVOLVEMENT**: `COPILOT-01` must continuously inspect 100% of assets from micro-details (period `.` size, pixels, tokens, syntax) to macro-architecture (whole app flow, security, fluid viewports, 5 Final Clearance Roles, and 7 Production-Readiness Dimensions).
- **STRICT ZERO-BYPASS ENFORCEMENT**: Omitting `COPILOT-01` from any task execution or workflow stage is strictly prohibited and classified as a severe system governance violation.
...
## Mandatory 5 Preventative Directives for All Future Software Projects

To permanently prevent mock data pollution, unverified email dispatches, and non-clickable UI entries, all agents and subagents in this workspace **MUST** enforce the following 5 Preventative Directives from Minute 1 of every project:

1. **Minute-1 Zero-Mock Clean Database Standard**: Every new web app MUST initialize with a 1-click **Clean Zero-Mock Database Ingest Mode** (`🔌 Real Live DB Ingest Mode`) by default. No static sample mock data shall pollute production testing.
2. **Clickable Hard-Copy Evidence File Standard**: Every list item, transaction row, security alert, or document log MUST be **100% interactive and clickable**, popping up an exact printable A4 tax statement or downloadable cryptographic proof file (`<TransactionEvidenceModal />`).
3. **Mandatory Pre-Dispatch Confirmation Gate**: Before executing ANY external network SMTP email dispatch, the Orchestrator MUST display the full email pitch draft in chat and wait for explicit user confirmation (`"isend na!"`).
4. **Direct Module Deep-Linking Standard**: Every application MUST include Hash Deep-Linking (`/#/view_name`) in `App.jsx` from Minute 1, exposing direct 1-click links in all proposal pitches (`https://gatzdevs.surge.sh/#/view_name`).
5. **Built-In Telemetry Error Logger & Email Alerts**: Every application MUST mount the `<TelemetryErrorLogger />` banner at the root layout from Minute 1 to harvest browser runtime exceptions and dispatch instant alerts to `mckinsyo01@gmail.com`.


## Core Directive: Uncompromising Product Integrity, Passion & Exhaustive 100% Inspection

To protect client trust and guarantee 10,000,000% perfection, all agents and subagents in this workspace **MUST** strictly obey the following Core Integrity Directive:

1. **Zero Tolerance for "Small" Flaws**: A single un-inspected foreign string, minor alignment glitch, or broken button destroys professional credibility. Never assume a small change is harmless.
2. **Exhaustive Automated Inspection Mandatory**: Before presenting ANY software product to the user or client, run exhaustive automated scripts and DevTools visual audits inspecting **100% of files, lines of code, typography, and interactive triggers**.
3. **Product Passion & WOW Factor**: Every product build must radiate craftsmanship, enthusiasm, and polish that instantly hooks the user and wows the client.

---

## 🛑 Mandatory Zero-Assumption Policy & Independent New Project Workspace Isolation

To permanently eliminate wasted AI quota, wasted time/effort, overwritten client apps, and un-clarified assumptions, all Master Orchestrators, Subagent Councils, and Task Specialists **MUST** strictly enforce the following 3 Hard Rules without exception:

1. **STRICT ZERO-ASSUMPTION POLICY**:
   - Never assume user intent, architectural boundaries, deployment targets, or project scope.
   - If any requirement, directory path, or workflow step is underspecified or ambiguous, **STOP IMMEDIATELY** and ask the user for explicit clarification. Guessing is strictly forbidden.

2. **MANDATORY INDEPENDENT NEW PROJECT WORKSPACE ISOLATION**:
   - Whenever the user initiates a *"Bagong Proyekto"* (New Project), the Orchestrator **MUST ALWAYS** create a brand-new, dedicated workspace directory (e.g., `c:\Users\Admin\.antigravity-ide\<NewProjectName>\`).
   - It is **STRICTLY PROHIBITED** to modify, overwrite, or mutate an existing completed client codebase (like `ReservaHost-AI` / `RevenueCat-AI`) when starting a new project.

3. **IMMUTABLE COMPLETED CLIENT CODEBASE PRESERVATION**:
   - Once a client software project is built, verified, zero-mock audited, and deployed, its repository is **PERMANENTLY IMMUTABLE**.
   - Re-using an existing project's `src/App.jsx` or overwriting its UI for another client is classified as a severe workflow violation. Every client gets their own 100% standalone, isolated repository.
