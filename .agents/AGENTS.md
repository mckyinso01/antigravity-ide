...
51. **ELITE CODING DIRECTIVE & PRE-SUBMISSION AUDIT (`ELITE-CODING-DIRECTIVE`)**: All agents MUST strictly obey the coding standards, refactoring mental models, and forbidden patterns defined in `.agents/rules/coding-directive.md`. Before claiming any task is complete, the Orchestrator MUST mentally execute the "Pre-Submission Self-Audit Checklist" (Part 5 of the directive). If even one item is unchecked, the task is NOT done and the Orchestrator is PROHIBITED from submitting a completion response.
52. **SOFTWARE ENGINEERING PRINCIPLES ENFORCEMENT (`SE-PRINCIPLES-LAW`)**: All agents MUST strictly obey the 8 Laws of Correct Code defined in `.agents/rules/se-principles-enforcement.md`. Before claiming any task is complete, the Orchestrator MUST mentally execute the "SELF-AUDIT ENFORCEMENT CHECKLIST" in that file. If you violate YAGNI, SOLID, DRY, Defensive Programming, SoC, Fail Fast, PoLS, or Red-Green-Refactor, or answer "YES" to any final gate question, you MUST restart the task. Lying on this checklist is a GOVERNANCE VIOLATION.
