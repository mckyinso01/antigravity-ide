...
interface SmartTooltipProps {
  ...
  stepNumber?: number;
  totalSteps?: number;
  smartBeaconType?: 'action' | 'clinical' | 'emergency' | 'fda';
...
export const SmartTooltip: React.FC<SmartTooltipProps> = ({
  ...
  const getBeaconBadge = () => {
    switch (smartBeaconType) {
      case 'emergency':
        return { dot: 'bg-rose-500', tag: 'STAT CODE BLUE' };
      case 'fda':
        return { dot: 'bg-purple-400', tag: 'FDA 21 CFR' };
      case 'clinical':
        return { dot: 'bg-emerald-400', tag: 'CLINICAL PROTOCOL' };
      default:
