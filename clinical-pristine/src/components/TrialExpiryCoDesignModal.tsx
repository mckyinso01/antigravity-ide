...
export const TrialExpiryCoDesignModal: React.FC<TrialExpiryCoDesignModalProps> = ({
  ...
  const handleFeedbackSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const message = `Clinical Pristine 7-Day Trial Feedback: Rating: ${feedbackRating}. Custom Requests: ${customRequest || 'None'}`;
      ...
      <div className="bg-[#0B132B] border border-cyan-500/50 rounded-3xl max-w-2xl w-full shadow-2xl overflow-hidden relative font-sans text-slate-100 flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white font-display flex items-center gap-2">
                <span>Clinical Pristine 7-Day Sovereign Access</span>
              ...
              <p className="text-xs text-slate-400 font-mono">
                {isExpired 
                  ...
                  : 'Evaluate spatial telemetry, MEWS scoring, and 1-click EHR ingestion with zero friction.'}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 text-xs font-mono">
          ...
          <div className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-white font-bold text-xs flex items-center gap-1.5 text-cyan-400">
                <MessageSquareQuote size={15} /> 1. How was your team's experience with Clinical Pristine?
              </span>
            ...
            <form onSubmit={handleFeedbackSubmit} className="space-y-3">
              ...
              <div>
                <label className="block text-[11px] text-slate-300 mb-1">
                  What custom reports, floorplan layouts, or EHR connectors would you like Founder Mharc Gatan to engineer for your hospital?
                </label>
                <textarea
                  value={customRequest}
                  onChange={(e) => setCustomRequest(e.target.value)}
                  placeholder="e.g. We need custom HL7 ADT-A08 bed swap triggers, Epic Hyperspace SSO bridge, and our hospital logo on wallboards..."
                  rows={2}
          ...
          <div className="p-4 rounded-2xl bg-gradient-to-r from-blue-950/40 via-slate-900 to-indigo-950/40 border border-blue-500/40 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-white font-bold text-xs flex items-center gap-1.5 text-blue-400">
                <ShieldCheck size={15} /> 2. Permanent Sovereign On-Premises Ownership
              </span>
              <span className="text-[10px] font-bold text-emerald-400">0% Monthly Per-Bed Tax</span>
            </div>
            <p className="text-slate-300 text-[11px] font-sans">
              Eliminate recurring Epic / Cerner subscription overhead. Deploy Clinical Pristine permanently behind your hospital firewall with 100% perpetual IP rights.
            </p>
          ...
          <div className="flex items-center justify-between p-3 rounded-xl bg-slate-950 border border-slate-800 text-[11px]">
            <div>
              <span className="text-slate-300 font-bold">Need more time to present to Clinical Leadership?</span>
        ...
        <div className="px-6 py-3 border-t border-slate-800 bg-[#070D1E] flex items-center justify-between text-[10px] font-mono text-slate-400">
          <span>🔒 100% Data Safety Guarantee: Your hospital records are never deleted or held hostage.</span>
          <button
            type="button"
            onClick={onClose}
