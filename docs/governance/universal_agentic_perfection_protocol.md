...
```mermaid
graph TD
    A[Subagent Executes Task] --> B[State 1: Builder Composition]
    B --> C[State 2: Adversarial Critic Self-Audit]
    C --> D{Passes All 15 YES-Gates?}
    D -- No (Any NO answer) --> B
    D -- Yes (15/15 YES) --> E[Submit to Council of Elders Review]
    E --> F{Council Unanimous Approval?}
    F -- Rejection/Feedback --> B
    F -- Unanimous Sign-Off --> G[Submit to Master Orchestrator]
    G --> H[OKF v0.2 Trust Verification & Telemetry Update]
    H --> I[Present Completed Platinum Work to User]
```
