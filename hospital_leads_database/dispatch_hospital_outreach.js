...
<head>
  ...
  <style>
    ...
    .footer { margin-top: 24px; border-top: 1px solid #e2e8f0; pt: 16px; font-size: 12px; color: #64748b; }
  </style>
</head>
<body>
  <div class="card">
...
async function verifyConnection() {
  console.log('🔄 Verifying SMTP connection to smtp.gmail.com...');
  await transporter.verify();
  console.log('✅ SMTP Connection Authenticated Successfully with mckinsyo01@gmail.com!');
}
...
async function sendTestEmail() {
  ...
  const sampleLead = {
    hospital_name: 'Salem Health',
    decision_maker: 'Bob MacDonald (CIO)',
    core_ehr: 'Epic',
    country: 'US',
    pain_point: 'High ED boarding volume; bed turnaround bottlenecks'
  };

  const { subject, plainText, html } = generateEmailContent(sampleLead);

  console.log(`📤 Sending TEST OUTREACH EMAIL to ${SENDER_EMAIL}...`);
  ...
  console.log('Recipient:', SENDER_EMAIL);
}

async function sendBatch(startId, endId, delayMs = 5000) {
  await verifyConnection();

  const leadsPath = path.join(__dirname, 'verified_100_us_uk_hospitals.json');
  const leads = JSON.parse(fs.readFileSync(leadsPath, 'utf8'));

  const selectedLeads = leads.filter(l => l.id >= startId && l.id <= endId);
  console.log(`📋 Found ${selectedLeads.length} leads in range #${startId} - #${endId}`);

  const logFile = path.join(__dirname, 'outreach_dispatch_log.json');
  ...
  for (const lead of selectedLeads) {
    ...
    console.log(`\n📨 Dispatching to #${lead.id}: ${lead.hospital_name} (${lead.decision_maker}) -> ${targetEmail}`);
    ...
    if (selectedLeads.indexOf(lead) < selectedLeads.length - 1) {
      console.log(`⏳ Waiting ${delayMs / 1000}s before next send to maintain high deliverability...`);
...
if (args.includes('--test')) {
  ...
  const end = parseInt(args[2], 10) || 5;
  sendBatch(start, end).catch(console.error);
} else {
  console.log('Usage: node dispatch_hospital_outreach.js --test OR --batch <startId> <endId>');
}
