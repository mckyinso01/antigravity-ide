...
export function auditMitnick(context = {}, mode = 'continuous') {
  ...
  findings.push({
    ...
    remediation: 'MFA step-up gate added to SalesReport.jsx and Settings.jsx (renders MfaChallengeModal when mfaVerified is false). Backend requireStepUp middleware wired to /tax-rates/validate and /writeoffs/validate routes. requireStepUp checks mfa_verified flag or fresh token (issued within 5 min).',
    verifiedAt: '2026-09-18',
    auditPass,
    verificationNote: 'SalesReport.jsx: if (!mfaVerified) return <MfaChallengeModal>. Settings.jsx: same gate. Backend security.js: requireStepUp added to tax-rates and writeoffs routes. AuthContext.verifyMfa calls /auth/mfa-stepup which issues mfa_verified token.',
  ...
  findings.push({
    ...
    status: 'remediated',
    remediatedAt: '2026-09-18',
    remediation: 'SupervisorOverrideModal wired into RefundModal with ₱1000 threshold. High-value refunds require dynamic OTP via backend /auth/supervisor-override endpoint (issues 5-min override token). apiClient.supervisorOverride(otp) called from modal.',
    verifiedAt: '2026-09-18',
    auditPass,
    verificationNote: 'RefundModal.jsx imports SupervisorOverrideModal, shows it when refundTotal > SUPERVISOR_OVERRIDE_THRESHOLD (₱1000) and no overrideToken. On approval, processRefund(token) completes the refund. Backend /auth/supervisor-override validates 6-digit OTP and issues short-lived token.',
  ...
  findings.push({
    ...
    status: 'remediated',
    remediatedAt: '2026-09-18',
    remediation: 'Mock fallback now defaults to cashier role (not admin). isOffline flag set when backend unreachable. App.jsx renders OFFLINE MODE banner when isOffline && isAuthenticated.',
    verifiedAt: '2026-09-18',
    auditPass,
    verificationNote: 'AuthContext.jsx login catch: setUser({role: cashier}) + setIsOffline(true). checkAuthState catch: same cashier default + isOffline. App.jsx line 106: {isOffline && isAuthenticated && banner "OFFLINE MODE — Backend unavailable. Running with limited cashier-only access."}',
