...
import { sumMoney, toCents, fromCents, subMoney, multiplyMoney, addMoney } from "@/lib/security/decimal";
import { wipeCanvas } from "@/lib/security/storageMonitor";
...
export default function SalesReport() {
  ...
  // Use integer-cents arithmetic for all financial aggregations (GEOHOT-05)
  const totalRevenue = sumMoney(periodTxns.map(t => t.total_amount || 0));
  const totalOrders = periodTxns.length;
  const totalCost = sumMoney(periodTxns.flatMap(t =>
    (t.items || []).map(i => multiplyMoney(i.unit_cost || 0, i.quantity || 0))
  ));
  ...
  periodTxns.forEach(t => {
    (t.items || []).forEach(item => {
      ...
      productSales[item.product_name].revenue = addMoney(productSales[item.product_name].revenue, item.subtotal || 0);
      productSales[item.product_name].cost = addMoney(productSales[item.product_name].cost, multiplyMoney(item.unit_cost || 0, item.quantity || 0));
    });
  });
  const topProducts = Object.entries(productSales)
    .map(([name, data]) => ({ name, ...data, profit: subMoney(data.revenue, data.cost) }))
  ...
  const generatePDF = () => {
    ...
    const pdfCanvas = doc.internal?.canvas;
    if (pdfCanvas) wipeCanvas(pdfCanvas);
    setGenerating(false);
  };
...
— OmniStock Automated Reports
  ...
  if (!mfaVerified) {
    return <MfaChallengeModal title="MFA Required — Sales Reports" />;
  }
