...
interface SmartTooltipProps {
  ...
  stepNumber?: number;
  totalSteps?: number;
  smartBeaconType?: 'action' | 'inventory' | 'dispatch' | 'rfid';
...
export const SmartTooltip: React.FC<SmartTooltipProps> = ({
  ...
  const getBeaconBadge = () => {
    switch (smartBeaconType) {
      case 'inventory':
        return { dot: 'bg-amber-400', tag: 'INVENTORY ALERT' };
      case 'dispatch':
        return { dot: 'bg-emerald-400', tag: 'DISPATCH ROUTE' };
      case 'rfid':
        return { dot: 'bg-purple-400', tag: 'RFID / BARCODE' };
      default:
            ...
            {nextStepAction && (
              <div className="p-2 rounded-xl bg-gradient-to-r from-cyan-950/70 to-blue-950/70 border border-cyan-500/40 text-[11px] font-sans">
                <div className="flex items-center gap-1.5 font-bold text-cyan-300 text-[10px] uppercase tracking-wider mb-0.5 font-mono">
                  <span>👉 Next Warehouse Action:</span>
                </div>
