...
export const BarcodeScannerModal: React.FC<BarcodeScannerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 font-sans">
      <div className="bg-[#070B14] border border-[#2A4374] rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col glow-mint">
        {/* Header */}
        <div className="h-14 border-b border-[#1E2D4D] bg-[#0D1527] px-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-mono font-bold text-sm text-white">Smart BYOD Barcode & RFID Scanner</h3>
              <span className="text-[10px] text-slate-400 font-mono">WASM Real-Time Video Engine • Sub-50ms</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button 
        ...
        <div className="p-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {skus.slice(0, 4).map((sku) => (
                <button
                  ...
                  className="p-2 rounded-xl bg-[#0D1527] hover:bg-[#121D36] border border-[#1E2D4D] hover:border-[#5BC0BE] text-left transition-all cursor-pointer font-mono text-[10px]"
                >
          ...
          <form onSubmit={handleManualSubmit} className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-[#0D1527] border border-[#1E2D4D] focus:border-[#5BC0BE] text-white text-xs rounded-xl pl-9 pr-3 py-2 outline-none font-mono"
              />
            </div>
            <button
              type="submit"
              className="bg-[#5BC0BE] hover:bg-[#6FFFE9] text-[#070B14] font-bold text-xs px-4 py-2 rounded-xl transition-all cursor-pointer font-mono"
            >
              Verify
            </button>
          </form>
          ...
          {matchedSku && (
            <div className="p-3 rounded-xl bg-[#121D36] border border-[#5BC0BE] space-y-1.5 animate-in fade-in zoom-in duration-150">
