...
export const WavePickOptimizerModal: React.FC<WavePickOptimizerModalProps> = ({
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-[480px] max-w-full z-50 bg-[#070B14]/98 border-l border-[#2A4374] shadow-[0_0_60px_rgba(0,0,0,0.9)] flex flex-col h-full overflow-y-auto animate-slideLeft font-sans">
      {/* Header */}
      <div className="h-14 border-b border-[#1E2D4D] bg-[#0D1527] px-5 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-mono font-bold text-sm text-white">Wave Pick Optimizer</h3>
            <span className="text-[10px] text-[#5BC0BE] font-mono">Eulerian Shortest-Path Walk Slasher</span>
          </div>
        </div>
        <button 
          onClick={onClose}
      ...
      <div className="p-5 space-y-4 font-mono text-xs flex-1 flex flex-col justify-between">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2.5">
            <div className="p-3 rounded-xl bg-[#0D1527] border border-[#1E2D4D]">
              <span className="text-[10px] text-slate-400 block mb-1">Pick Travel</span>
              <div className="flex items-center gap-1.5 text-emerald-400 font-bold text-sm">
                <TrendingDown size={16} />
                <span>-38.4%</span>
              </div>
              <span className="text-[9px] text-slate-500">Saved 420m</span>
            </div>
            <div className="p-3 rounded-xl bg-[#0D1527] border border-[#1E2D4D]">
              <span className="text-[10px] text-slate-400 block mb-1">Cycle Time</span>
              <div className="flex items-center gap-1.5 text-[#5BC0BE] font-bold text-sm">
                <Clock size={16} />
                <span>11.4m</span>
              </div>
              <span className="text-[9px] text-slate-500">vs 19.8m std</span>
            </div>
            <div className="p-3 rounded-xl bg-[#0D1527] border border-[#1E2D4D]">
              <span className="text-[10px] text-slate-400 block mb-1">Batch Vol</span>
              <div className="flex items-center gap-1.5 text-white font-bold text-sm">
                <Layers size={16} />
                <span>4 SKUs</span>
              </div>
              <span className="text-[9px] text-slate-500">1 Cart</span>
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-[#0D1527] border border-[#1E2D4D] space-y-2">
        ...
        <div className="flex flex-col gap-2 pt-3 border-t border-[#1E2D4D]">
          <button
            onClick={handleRunOptimization}
            className="w-full bg-[#121D36] hover:bg-[#1E2D4D] border border-[#2A4374] text-slate-200 py-2.5 rounded-xl font-bold transition-all cursor-pointer flex items-center justify-center gap-2"
          >
            <Zap size={14} className="text-[#5BC0BE]" />
            <span>Re-calculate Shortest Path</span>
          </button>
          <button
            onClick={handleDispatch}
            className="w-full bg-gradient-to-r from-[#5BC0BE] to-[#3A86FF] hover:opacity-90 text-[#070B14] py-3 rounded-xl font-bold transition-all cursor-pointer flex items-center justify-center gap-2 shadow-lg glow-mint"
          >
            <Truck size={15} />
            <span>Dispatch Wave to Floor Pickers</span>
          </button>
        </div>
      </div>
    </div>
