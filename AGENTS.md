...
- Security Scenarios Tab: 7 security-focused role personas (CISO, Security Engineer, Incident Responder, Penetration Tester, Compliance Officer, SOC Analyst, Data Protection Officer) with multi-scenario coverage across incident response, threat modeling, and compliance audit workflows. Each role has 3 scenario types (filterable via pills), 5-Titan adversarial threat matrix with Zod defense validators, and cross-role collision scenarios. Data: `omnistock-app/src/agents/audits/securityRoleScenarios.js`. Component: `omnistock-app/src/components/agents/SecurityScenariosTab.jsx`. Tab added to ProductionEngine between 'roles' and 'devils-team'.
- F&B Management Scenarios Tab: 4 F&B management role personas (F&B Director, Restaurant Manager, Purchasing Manager, QA Manager) with multi-scenario coverage across 3 operation types (Fine Dining, QSR/Fast Food, Hotel F&B/Banquet) and 3 threat categories (POS & Payment Fraud, Inventory & Supply Chain, Food Safety & Compliance). Each role has 3 scenarios (one per operation type), 5-Titan adversarial threat matrix with 6 new Zod defense validators (posReconciliation, supplierOrder, haccpCompliance, banquetAllocation, franchiseCompliance, coldChain), and 3 cross-role collision scenarios. Data: `omnistock-app/src/agents/audits/fbManagementScenarios.js`. Component: `omnistock-app/src/components/agents/FBManagementScenariosTab.jsx`. Tab added to ProductionEngine between 'security' and 'devils-team'.
