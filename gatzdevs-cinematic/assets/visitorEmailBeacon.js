// 📡 GATZ DEVS & SYSTEMS STUDIO: REAL-TIME VISITOR TELEMETRY & EMAIL BEACON
...
(function() {
  ...
  async function sendEmailBeacon(eventType, details = {}) {
    ...
    const payload = {
      _subject: eventType === 'PAGE_VISIT' 
        ? `🚨 [GatzDevs Lead] New Visitor from ${geo.city}, ${geo.country}`
  ...
  if (!sessionStorage.getItem("gatz_visitor_beacon_sent")) {
    sessionStorage.setItem("gatz_visitor_beacon_sent", "true");
    setTimeout(() => {
  ...
  document.addEventListener("click", function(e) {
    ...
    if (href.includes("clinical-pristine") || text.includes("Launch Demo")) {
      ...
      if (!sessionStorage.getItem("gatz_demo_beacon_sent")) {
        sessionStorage.setItem("gatz_demo_beacon_sent", "true");
