...
export function useUrlProspectSession(vertical: string = 'healthcare_legal'): ProspectSessionInfo {
  const [prospectSession] = useState<ProspectSessionInfo>(() => {
    try {
      const params = new URLSearchParams(window.location.search);
      const urlOrg = params.get('org') || params.get('hospital') || params.get('payer') || '';
